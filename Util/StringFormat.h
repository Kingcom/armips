...
void doFormatString(std::stringstream& stream, const char* format);
bool doFormatSetup(std::stringstream& stream, const char*& format);

template <typename First, typename... Rest>
void doFormatString(std::stringstream& stream, const char* format, First& first, const Rest&... rest)
{
	while (*format != 0)
	{
		if (*format == '%')
...
template <typename... Args>
std::string formatString(const char* format, const Args&... args)
{
