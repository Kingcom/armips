...
const char ArmConditions[16][3] = {"eq", "ne", "cs", "cc", "mi", "pl", "vs", "vc",
								   "hi", "ls", "ge", "lt", "gt", "le", "",   "nv"};
const char ArmAddressingModes[4][3] = {"da", "ia", "db", "ib"};
const char ArmShiftModes[4][4] = {"lsl", "lsr", "asr", "ror"};

const char ArmPsrModes[16][5] = {"_???", "_ctl", "_x",  "_xc",  "_s",  "_sc",  "_sx",  "_sxc",
...
bool CArmInstruction::Validate()
{
	...
	if (Vars.Shift.UseShift == true && Vars.Shift.ShiftByRegister == false)
	{
		...
		if (num == 0 && mode == ARM_SHIFT_LSR)
			mode = ARM_SHIFT_LSL;
		else if (num == 0 && mode == ARM_SHIFT_ASR)
			mode = ARM_SHIFT_LSL;
		else if (num == 0 && mode == ARM_SHIFT_ROR)
			mode = ARM_SHIFT_LSL;
		else if (num == 32 && mode == ARM_SHIFT_LSR)
			num = 0;
		else if (num == 32 && mode == ARM_SHIFT_ASR)
			num = 0;
			...
			if ((temp = getShiftedImmediate(Vars.Immediate, Vars.Shift.ShiftAmount)) == -1)
			{
				...
				if ((Opcode.flags & ARM_OPTIMIZE) && (temp = getShiftedImmediate(~Vars.Immediate, Vars.Shift.ShiftAmount)) != -1)
				{
					if (Opcode.flags & ARM_OPMOVMVN)
						Vars.Opcode.NewEncoding = Opcode.encoding ^ 0x0400000;
					if (Opcode.flags & ARM_OPANDBIC)
						Vars.Opcode.NewEncoding = Opcode.encoding ^ 0x1C00000;
					if (Opcode.flags & ARM_OPCMPCMN)
						Vars.Opcode.NewEncoding = Opcode.encoding ^ 0x0200000;
					Vars.Opcode.UseNewEncoding = true;
				}
				else
				{
...
void CArmInstruction::Encode() const
{
	...
	if ((Opcode.flags & ARM_UNCOND) == 0)
		encoding |= Vars.Opcode.c << 28;
	if (Vars.Opcode.s == true)
		encoding |= (1 << 20);
	...
	case ARM_TYPE7: // ARM.7: Multiply and Multiply-Accumulate (MUL,MLA)
		...
		if (Opcode.flags & ARM_N)
			encoding |= (Vars.rn.num << 12);
		encoding |= (Vars.rs.num << 8);
		if ((Opcode.flags & ARM_Y) && Vars.Opcode.y == true)
			encoding |= (1 << 6);
		if ((Opcode.flags & ARM_X) && Vars.Opcode.x == true)
			encoding |= (1 << 5);
		encoding |= (Vars.rm.num << 0);
		break;
	case ARM_TYPE9: // ARM.9: Single Data Transfer (LDR, STR, PLD)
		if (Vars.writeback == true)
			encoding |= (1 << 21);
		if (Opcode.flags & ARM_N)
			encoding |= (Vars.rn.num << 16);
		if (Opcode.flags & ARM_D)
			encoding |= (Vars.rd.num << 12);
		if ((Opcode.flags & ARM_SIGN) && Vars.SignPlus == false)
			encoding &= ~(1 << 23);
		if ((Opcode.flags & ARM_ABS) && Vars.negative == true)
			encoding &= ~(1 << 23);
		if (Opcode.flags & ARM_IMMEDIATE)
		{
	...
	case ARM_TYPE11: // ARM.11: Block Data Transfer (LDM,STM)
		if (Opcode.flags & ARM_LOAD)
			encoding |= (LdmModes[Vars.Opcode.a] << 23);
		else if (Opcode.flags & ARM_STORE)
			encoding |= (StmModes[Vars.Opcode.a] << 23);
		if (Vars.psr == true)
			encoding |= (1 << 22);
		if (Vars.writeback == true)
			encoding |= (1 << 21);
