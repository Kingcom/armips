...
bool GenericAssemblerFile::open(bool onlyCheck)
{
	...
	if (!onlyCheck)
	{
		...
		case Open:
			...
			if (!stream.is_open())
			{
				Logger::printError(Logger::FatalError,L"Could not open file {}",fileName.wstring());
				return false;
			}
			return true;
		...
		case Create:
			...
			if (!stream.is_open())
			{
				Logger::printError(Logger::FatalError,L"Could not create file {}",fileName.wstring());
				return false;
			}
			return true;
		...
		case Copy:
			...
			{
				Logger::printError(Logger::FatalError,L"Could not copy file {}",originalName.wstring());
				return false;
			}
	...
	case Open:
		...
		if (!temp.is_open())
		{
			Logger::queueError(Logger::FatalError,L"Could not open file {}",fileName.wstring());
			return false;
		}
		temp.close();
		return true;
	...
	case Create:
		...
		if (!temp.is_open())
		{
			Logger::queueError(Logger::FatalError,L"Could not create file {}",fileName.wstring());
			return false;
		}
		temp.close();
	...
	case Copy:
		...
		if (!temp.is_open())
		{
			Logger::queueError(Logger::FatalError,L"Could not open file {}",originalName.wstring());
			return false;
		}
		temp.close();
